...
struct DebouncedSlider<Label, ValueLabel> : View where Label : View, ValueLabel : View {
    ...
    init(value: Binding<Double>, debounceInterval: Double = 0.1, @ViewBuilder label: () -> Label, @ViewBuilder minimumValueLabel: () -> ValueLabel, @ViewBuilder maximumValueLabel: () -> ValueLabel) {
        self._value = value
...
extension DebouncedSlider {
    ...
    private class DebouncedSliderViewModel : ObservableObject {
        ...
        init(value: Double, debounceInterval: Double) {
            self.value = value
